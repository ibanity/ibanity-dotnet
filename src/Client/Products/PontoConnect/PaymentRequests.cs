...
namespace Ibanity.Apis.Client.Products.PontoConnect
{
    ...
    /// </summary>
    public class PaymentRequests : ResourceWithParentClient<PaymentRequestResponse, object, object, PaymentLinks, Token>, IPaymentRequests
    {
        ...
        /// <inheritdoc />
        public Task<PaymentRequestResponse> Create(Token token, Guid accountId, PaymentRequest payment, Guid? idempotencyKey, CancellationToken? cancellationToken)
        {
            if (token is null)
    ...
    /// <summary>
    /// <para>This is an object representing a payment request. When you want to initiate payment request from one of your user's accounts, you have to create one to start the authorization flow.</para>
    /// <para>If you provide a redirect URI when creating the payment request, you will receive a redirect link to send your customer to to start the authorization flow. Note that for live payment request, your user must have already requested and been granted the payment request service for their organization to use this flow.</para>
    /// <para>When authorizing payment request initiation in the sandbox, you should use the pre-filled credentials and 123456 as the digipass response.</para>
    /// </summary>
    public interface IPaymentRequests
    {
        ...
        Task<PaymentRequestResponse> Create(Token token, Guid accountId, PaymentRequest paymentRequest, Guid? idempotencyKey = null, CancellationToken? cancellationToken = null);
